...
import { PriorityLevelItem, MasterDataStatus } from '../types';
import { useUpdatePriorityMutation } from '../api';
...
export const EditPriorityModal: React.FC<EditPriorityModalProps> = ({
  ...
  useEffect(() => {
    if (priority) {
      ...
      setSortOrder(priority.sortOrder);
      setStatus(priority.status);
      setColor(getPriorityColor(priority.id, priority.label));
      setErrorMessage('');
    }
  }, [priority]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await updatePriorityMutation.mutateAsync({
        id: priority.id,
        data: {
          label: label.trim(),
      ...
      setPriorityColor(priority.id, color, updated.label);

      onSuccess?.(`Priority level "${updated.label}" updated successfully.`);
      onClose();
    } catch (err: any) {
      const msg =
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-[#64748B] mt-0.5">
                Update label, order ranking, or active state
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-1">
              Controls display order in ticket dropdowns. Lower numbers appear first.
            </p>
          </div>
