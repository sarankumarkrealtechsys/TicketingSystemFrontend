...
import { TicketStatusItem, TicketStatusBehavior, MasterDataStatus } from '../types';
import { useUpdateTicketStatusMutation } from '../api';
...
export const EditStatusModal: React.FC<EditStatusModalProps> = ({
  ...
  useEffect(() => {
    if (statusItem) {
      ...
      setSortOrder(statusItem.sortOrder);
      setStatus(statusItem.status);
      setColor(getStatusColor(statusItem.id, statusItem.behavior, statusItem.label));
      setErrorMessage('');
    }
  }, [statusItem]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save custom color keyed by both ID and updated label
      setStatusColor(statusItem.id, color, updated.label);

      onSuccess?.(`Workflow status "${updated.label}" updated successfully.`);
      onClose();
    } catch (err: any) {
      const msg =
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-[#64748B] mt-0.5">
                {statusItem.team ? `Team: ${statusItem.team.name}` : 'Global Status'}
              </p>
            </div>
          </div>
          <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
            <div>
              ...
              <SelectDropdown<TicketStatusBehavior>
                ...
                onChange={(val) => {
                  setBehavior(val);
                  setColor(getStatusColor(statusItem.id, val, label));
                }}
            ...
            <div className="flex items-center gap-3">
              ...
              <label
                className={`flex items-center gap-2 text-xs font-bold text-gray-700 ${
                  statusItem.isDefault ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'
                }`}
                title={statusItem.isDefault ? 'System default statuses cannot be archived' : undefined}
              >
                <input
                  type="radio"
                  name="edit-status-mode"
                  value="INACTIVE"
                  disabled={statusItem.isDefault}
